...
interface Props {
  open: boolean
  onClose: () => void
  onImport: (produtos: { codloja: string; codproduto: string; qtde_facing: number }[]) => Promise<{
    success: string[]
...
const ImportacaoFacing: React.FC<Props> = ({ open, onClose, onImport, onDownloadLayout }) => {
  const [produtos, setProdutos] = useState<{ codloja: string; codproduto: string; qtde_facing: number }[]>([])
    ...
    reader.onload = (e) => {
      try {
        ...
        const itens = jsonData
          .map((row) => ({
            codloja: row.codloja?.toString().trim().padStart(2, "0"),
            codproduto: row.codproduto?.toString().trim().padStart(5, "0"),
            qtde_facing: Number(row.qtde_facing),
          }))
          .filter((row) => row.codloja && row.codproduto && !Number.isNaN(row.qtde_facing))

        if (!itens.length) {
          setError("Nenhuma linha válida encontrada. Use o layout com colunas codloja, codproduto e qtde_facing.")
        } else {
          setProdutos(itens)
          setError(null)
        }
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="md">
      <DialogTitle>Importar Facing em Massa</DialogTitle>
      <DialogContent>
        <Typography sx={{ mb: 2 }}>Planilha esperada: codloja, codproduto, qtde_facing.</Typography>
        ...
        <Box
          ...
          onClick={() => document.getElementById("file-facing")?.click()}
        >
          <input
            id="file-facing"
            type="file"
...
export default ImportacaoFacing
